...
G.gc = util.getParameter("groupuin");// || client.getGroupUin(); //群号 这里没有必要从客户端再取了....接口是不存在的...尼玛..
...
G.codeValueFilter = function(string) {
    string += '';
    string = string.escHtmlEp();
    return string.replace(new RegExp("<", "gm"), "[").replace(new RegExp(">", "gm"), "]");
};
...
var newui2= 5521;
// if(G.version >= newui){
//     $('#member-box').addClass('old-version');
// }
if(G.version >= newui && G.version < newui2){
    $('#member-box').addClass('old-version');
}else if(G.version >= newui2 ){
    $('#member-box').addClass('new-version');
}
...
G.memberData = {
    itemsload: false,
    labelload: false,           //成员标签
    labelflagload: false,       //是否显示标签
    groupload: false,           //群相关设置是否已经加载完成
    face: {},
    memberMap : {},
    status: {
        1: '老师',
...
var deflevel = {"lvln1":"潜水","lvln10":"一见倾心","lvln11":"超凡脱俗","lvln12":"风华绝代","lvln13":"崭露头角","lvln14":"金玉满堂","lvln15":"富甲一方","lvln2":"冒泡","lvln3":"吐槽","lvln4":"活跃","lvln5":"话唠","lvln6":"传说"}
...
G.memberData.levelname = deflevel;  //先设置一下默认的等级, 后面拉到数据之后再更新下这儿了.
G.loadReady = true;//是否已经正确的从客户端取到相关数据了.
window.G = G;
...
if(userlist){
    var qqlist;
    if (G.officemode == "0") {
        qqlist = client.getMemberInfoList(userlist);
    }else{
        qqlist = client.getMemberOfficeInfoList(userlist);
    }
    ...
    */
    ...
    for (var i = 0; i < len; i++) {
        ...
        if(qq && qqdata){
            if (G.officemode == "0") {
                var card = G.codeFilter(qqdata["cardName"]) || "",
                    levelname = (qqdata["levelLable"]) || "",
                    level = qqdata['levelValue']+1,
                    nick = G.codeFilter(qqdata["nickName"]) || qq,
                    point = (qqdata["levelPoint"]) || 0;
               item = {
                    card: card,
                    levelname: levelname,
                    levelid : level,
                ...
                var card = G.codeFilter(qqdata["cardName"]) || "",
                    nick = G.codeFilter(qqdata["nickName"]) || "",
                    tel = G.codeFilter(qqdata["tel"]) || "",
                    position = G.codeFilter(qqdata["position"]) || "",
                    gender = G.codeFilter(qqdata["gender"]) || "";
                item = {
                    qq : qq,
                    card : card,
                    nick : nick,
        ...
        if (G.officemode == "0") {
            G._memberData.items.push(item);
        }else{
            G._memberData.office_items.push(item);
        }
        G.memberData.memberMap[qq] = item;
...
},{"../lib/config":3,"../lib/report":4,"../lib/util":7,"../module/member/card":8,"../module/member/client":10,"../module/member/list-data":13,"../module/member/list-html":14,"../module/member/tmpl/member-list-box":20,"../module/member/tmpl/member-list-one":21,"../module/member/tmpl/member-thead":22,"../module/member/tmpl/office-member-list-one":23}],2:[function(require,module,exports){
'use strict';
/*
...
},{"./callClient":2,"./util":7}],5:[function(require,module,exports){
'use strict';
//ajax请求
var util = require('./util');
...
db.post = post;
db.bkn = bkn;
},{"./report":4,"./requestrep":6,"./util":7}],6:[function(require,module,exports){
'use strict';
/**
...
},{"./report":4}],7:[function(require,module,exports){
'use strict';
//一些工具方法
...
},{"../../lib/util.js":7,"./cgi":9,"./client":10}],9:[function(require,module,exports){
'use strict';
...
module.exports = (function() {
    ...
    function setCard(param, cb, error) {
        ...
        /*
 24 #define EC_DIRTY_NAME  (11)   名称不合法
 25 #define EC_DIRTY_PHONE (12)
 26 #define EC_DIRTY_MAIL  (13)
 27 #define EC_DIRTY_REMARK (14)        
         */
    ...
    function getMembers(param, cb, error) {
        ...
        if(typeof nodeData !== 'undefined' && nodeData['member'] && nodeData['member'].ec === 0){
            cb(nodeData['member']);
        }else{
    ...
    //去掉冗余数据的cgi
    function getMembersNew(param,cb,error){
        if (!param.gc) {
            param.gc = G.gc;
            // param.n = 20;
            // param.p = 1;
        ...
        if(typeof nodeData !== 'undefined' && nodeData['member'] && nodeData['member'].ec === 0){
            ...
            request.get('/cgi-bin/qun_info/get_members_info_v1', param, cb, error);
        }
    }
    ...
    function setAdmin(param, cb, error) {
        ...
        /*
        错误码:
          9 #define EC_INVAILD_GC 11
         10 #define EC_INVAILD_UIN 12  uin错误.
         11 #define EC_MAX_ADMIN 13
         12 #define EC_NO_RIGHT 14     非群成员.
         */
...
},{"../../lib/config":3,"../../lib/report":4,"../../lib/request":5,"./client":10,"./util":28}],10:[function(require,module,exports){
'use strict';
//一些工具方法
//设置页用到的客户端方法
...
module.exports = (function() {
    ...
    var getMemberInfoList = function(list) {
        var d = client.callClient("GetSomeMemberName", ['{\"uins\":[' + list.join(',') + ']}']);
        if (d) {
            return d;
        } else {
            return false;
    ...
    var getMemberOfficeInfoList = function(list) {
            var d = client.callClient("GetSomeMemberNameOnOfficeMode", ['{\"uins\":[' + list.join(',') + ']}']);
            if (d) {
                return d;
            } else {
                return false;
    ...
    return {
        ...
        getMemberOfficeInfoList : getMemberOfficeInfoList,
...
},{"../../lib/callClient":2,"../../lib/util":7}],11:[function(require,module,exports){
'use strict';
...
},{"../../lib/config":3,"../../lib/util":7,"./card":8,"./cgi":9,"./client":10,"./label":12,"./status":15,"./tmpl/content-menu":16}],12:[function(require,module,exports){
'use strict';
...
},{"./cgi":9,"./tmpl/label-menu":19}],13:[function(require,module,exports){
'use strict';
...
module.exports = (function() {
    ...
    var createMemberList = function() {
        //console.log(' member office mode is ',G.officemode == '0');
        //普通模式
        if (G.officemode == "0") {
            //家校群数据还未返回
            ...
            if (G.memberData.itemsload && G.memberData.labelload && G.memberData.groupload && G.memberData.labelflagload) {
                ...
                //console.log('indentity',G.selfIndentity);
                //成员权限
                /*
                去掉by horde
                在处理cgi返回数据的时候就同时处理掉权限这块了.
    ...
    var init = function() {
        ...
        if (G.memberData.groupload) {} else {
            cgi.getGroupInfo({}, function(d) {
                ...
                if (d.ec == 0) {
                    ...
                    G.isSelfCustumer = (d && d.app_privilege_flag) & 0x4000;
        ...
        if (G.officemode == "0") {
            ...
            if (G.memberData.itemsload) {} else {
                ...
                cgi.getMembersNew(param, function(d) {
                    setTimeout(function() {
                    ...
                    if (d.ec == 0) {
                        // G.memberData.level = 0; //原来这个是啥?
                        // G.memberData.type = 1;  //这个又是啥?
                        G.memberData.level = d.level;
                        G.memberData.type = d.type;

                        G.memberData.max_admin = d.max_admin || util.getAdminNum(d.max_num);
                        G.memberData.ext_num = d.ext_num;
                        G.memberData.shutup_list = {};//禁言列表;
                        // G.memberData.level = d.level;
                        // G.memberData.type = d.type;
                        // G.memberData.ext_num = d.ext_num;
                        ...
                        */
                        ...
                        if(d.adm){
                            mlist = [d.owner].concat(d.adm);
                        }else{
                            mlist = [d.owner];
                        }
                        //管理员个数
                        G.memberData.manage_num = mlist.length;

                        for(var i in mlist){
                           var qq = mlist[i];
                            var admData = G.memberData.memberMap[qq];
                            var cgiData = d.members[qq];
                            admData.labelid = "-1";
                            if(i == 0){
                                admData.indentity = 'owner';
                            }else{
                                admData.indentity = 'admin';
                            }
                            admData.idx = i;
                            if(cgiData.lst){
                                admData.times = memberUtil.dateFormat(new Date(parseInt(cgiData.lst) * 1000), "yyyy/MM/dd");
                            }
                            //填好友关系.
                            if(cgiData.fr){
                                friends[qq] = 1;
                            ...
                            if(cgiData.ll){
                                admData.levelid = cgiData.ll;
                            }
                            if(cgiData.lp){
                                admData.point = cgiData.lp;
                            }
                            if(cgiData.cd){
                                admData.card = cgiData.cd;
                            }
                            if(cgiData.nk){
                                admData.nick = cgiData.nk;   
                            }                            
                            G.memberData.items.push(admData);
                        }

                        var stIdx = mlist.length;
                        ...
                        if(d.members){
                            for(var i in d.members){
                                var qq = i;
                                var cgiData = d.members[i];
                                var data = G.memberData.memberMap[qq];
                                //不在管理员列表中的数据直接添加.
                                //console.log(typeof qq,qq,mlist,$.inArray(qq,mlist));
                                if($.inArray(parseInt(qq),mlist) < 0){
                                    var item = G.memberData.memberMap[qq];
                                        item.blackuser = cgiData.cf || 0;
                                        item.idx = stIdx++;
                                    if(cgiData.lst){
                                        item.times = memberUtil.dateFormat(new Date(parseInt(cgiData.lst) * 1000), "yyyy/MM/dd");
                                    }
                                    //填好友关系.
                                    if(cgiData.fr){
                                        friends[qq] = 1;
                                    ...
                                    //禁言的时间
                                    if(cgiData.sut){
                                        G.memberData.shutup_list[qq] = cgiData.sut;
                                    }
                                    if(cgiData.ll){
                                        item.levelid = cgiData.ll;
                                    }
                                    if(cgiData.lp){
                                        item.point = cgiData.lp;
                                    }
                                    if(cgiData.cd){
                                        item.card = cgiData.cd;
                                    }
                                    if(cgiData.nk){
                                        item.nick = cgiData.nk;   
                                    } 
                                    item.indentity = '';
                                    G.memberData.items.push(item);
                                }
                            }
                        }else{
                            for(var qq in G.memberData.memberMap){
                                if($.inArray(qq,mlist) < 0){
                                    var item = G.memberData.memberMap[qq]; 
                                        item.blackuser = 0;
                                        item.idx = stIdx++;
                                    G.memberData.items.push(item);
                        ...
                        //items数据组装
                        //首先插入群主,然后插入管理员,再插入普通成员
                        //管理员列表
                        //群主放在第一位
                        /*
                        var mlist = [d.owner].concat(d.adm);
                        ...
                        for(var i = 0,l = mlist.length;i<l;i++){
                            ...
                            admData.idx = i;
                            if(d.times[qq]){
                                admData.times = memberUtil.dateFormat(new Date(parseInt(d.times[qq]) * 1000), "yyyy/MM/dd");
                            }
                            G.memberData.items.push(admData);
                        }

                        var stIdx = d.adm.length;
                        //i = 0, len = d.mems.length;
                        var len = d.mems.length;
                        //有返回 member列表
                        if(len){
                            var t0 = new Date().getTime();
                            for(var i = 0;i<len;i++){
                                ...
                                if($.inArray(qq,mlist) < 0){
                                    var item = G.memberData.memberMap[qq];
                                        item.blackuser = d.mems[i].b;
                                        item.idx = stIdx++;
                                    G.memberData.items.push(item);
                                }
                                
                            }
                        }else{ //如果cgi的返回有问题..直接使用map中的数据..会有一些差异.
                            for(var qq in G.memberData.memberMap){
                                if($.inArray(qq,mlist) < 0){
                                    var item = G.memberData.memberMap[qq]; 
                                        item.blackuser = 0;
                        ...
                        */
                        /*else{
                            ...
                            for(var i in d.join){
                                ...
                                var data = G.memberData.memberMap[qq];
                                if(d.times[qq]){
                                    data.times = memberUtil.dateFormat(new Date(parseInt(d.times[qq]) * 1000), "yyyy/MM/dd");
                                }  
                                G.memberData.items.push(data);
                            }
                        }
                        */

                        /*
                        5000人群这里很可能没有数据了...有需要做个兼容.
                        排序的时候也需要把群主,管理员直接排在最前面.
                        */
                        /*
                        for (i = 0; i < len; i++) {
...
},{"../../lib/report":4,"../../lib/util":7,"./cgi":9,"./list-html":14,"./toolbar":27,"./util":28}],14:[function(require,module,exports){
'use strict';
...
module.exports = (function() {
    ...
    //默认管理员排序
    //先删除原有的管理员dom,然后统一在最前面添加.并遍历一次dom修改i属性
    var sortByIndentity = function() {
        var domList = $("#member-tbody li");
        var list = G.memberData.items || G.memberData.office_items;
        var mList = list.slice(0,G.memberData.manage_num);
        var html = '';
        for(var i = 0,l = mList.length;i<l;i++){
            var item = mList[i],
                qq = item.qq;
            $('#qq'+qq).remove();
            html += getDomHtml(qq,i);
        }
        $("#member-tbody").prepend(html);
    };
    //设置头像
    ...
    var loadTbody = function(ref) {
        ...
        //一定重载 2016-6-2 由于改过了cgi并且,客户端的等级可能有误的情况下为了保证数据显示是ok的.必定要重载一次.
...
},{"../../lib/config":3,"../../lib/report":4,"../../lib/util":7,"./card":8,"./cgi":9,"./client":10,"./content-menu":11,"./label":12,"./status":15,"./tmpl/homeschool-member-list-one":17,"./tmpl/homeschool-member-thead":18,"./tmpl/member-list-box":20,"./tmpl/member-list-one":21,"./tmpl/member-thead":22,"./tmpl/office-member-list-one":23,"./tmpl/office-member-thead":24}],15:[function(require,module,exports){
'use strict';
...
},{"../profile/report":29,"./cgi":9}],16:[function(require,module,exports){
'use strict;'
var _ = {};
var escapeMap = {
    '&': '&amp;',
...
var shut     = locals.shut;
var isfriend = locals.isfriend;
var version  = locals.version;
var shield   = locals.shield;
var menu     = locals.menu;
;
__p += '\r\n<ul class="context-list" tabindex="0" aria-label="功能菜单">\r\n    ';
if(G.selfUin!=qq){;
...
((__t = (qq)) == null ? '' : __t) +
'" tabindex="-1"/>\r\n    <!-- <div class="member-list-one-item">\r\n        <div class="homeschool-member-info"> -->\r\n            <div class="ico"><button data-setadmin="' +
...
var qq = locals.qq;
var listOne = locals.listOne;
var i = locals.i;
var cgiready = locals.cgiready;
;
__p += '\r\n<li id="qq' +
((__t = (qq)) == null ? '' : __t) +
'" data-member-list="' +
...
var nick = G.codeFilter(items[i].nick).replace('"','＂');
;
__p += '\r\n\r\n<div id="list-' +
((__t = (qq)) == null ? '' : __t) +
'" data-isfriends="' +
...
'" src="http://3.url.cn/qun/clt/qinfo/member/img/shield.png" alt=""><span class="txt">' +
((__t = (G.codeFilter(items[i].nick))) == null ? '' : __t) +
'</span><span class="blackuser" title="不良记录成员">!</span>\r\n            </div>\r\n        <!-- </div> -->\r\n        <div class="card">\r\n            <span class="txt" id="nick-txt-' +
((__t = (qq)) == null ? '' : __t) +
'">' +
((__t = (G.codeFilter(items[i].card))) == null ? '' : __t) +
'</span><input name="nick-input" type="text" maxlength="60" id="nick-input-' +
((__t = (qq)) == null ? '' : __t) +
'" value="' +
((__t = (G.codeValueFilter(items[i].card))) == null ? '' : __t) +
...
if((G.selfIndentity=="owner") && !G.isSelfCustumer){;
__p += '<a id="editGrade" class="edit" href="#" title="编辑等级" aria-label="编辑等级"></a>';
};
__p += '<a id="gradeHelp" class="help" href="javascript:void(0);" title="等级规则" aria-label="等级规则" tabindex="-1"></a></li>\r\n    <li class="levelpoint" data-record="point"><span>积分</span><i class="sort"></i></li>\r\n    <li class="time" data-record="times"><span>最后发言</span><i class="sort"></i></li>\r\n\t\r\n</ul>';
return __p
}
...
'" data-blackuser="' +
((__t = (G.memberData.blackMod==true?items[i].blackuser:0)) == null ? '' : __t) +
'">\r\n   <!--  <div class="member-list-one-item">\r\n        <div class="office-member-one-info"> -->\r\n            <div class="ico"><button data-setadmin="' +
((__t = (G.selfIndentity)) == null ? '' : __t) +
'" class="' +
((__t = (items[i].indentity)) == null ? '' : __t) +
' ' +
...
((__t = (qq)) == null ? '' : __t) +
'" value="' +
((__t = (G.codeValueFilter(items[i].tel))) == null ? '' : __t) +
...
((__t = (qq)) == null ? '' : __t) +
'" value="' +
((__t = (G.codeValueFilter(items[i].position))) == null ? '' : __t) +
...
var clientkey = locals.clientkey;
...
if(G.memberData.max_admin){
    _totalAdmin=G.memberData.max_admin;
}else{
...
var group = locals.group;
var clientkey = locals.clientkey;
var canAddMenber = locals.canAddMenber;
if(G.selfUin==group.gOwner){
    _flag=true;
}
var _i=0,_len=group.gAdmins?group.gAdmins.length:0;
...
if(_flag){;
__p += '\r\n        <div class="manage-item"><a data-managelog="true" href="javascript:void(0);" tabindex="0" title="管理记录" aria-label="管理记录">管理记录</a></div>\r\n        ';
};
__p += '\r\n        <div class="manage-item" style="display:none;"><a data-blackuser="true" href="javascript:void(0);" tabindex="0" title="不良成员" aria-label="不良成员" >不良成员(<span id="blackuser-count">-</span>)</a></div>\r\n        ';
if(_flag){;
__p += '\r\n        <div class="manage-item"><a id="advManage" href="' +
((__t = (advManage)) == null ? '' : __t) +
'" target="blank" title="高级管理" aria-label="高级管理">高级管理</a></div>\r\n        ';
};
__p += '\r\n        <div class="manage-item"><a id="edit-nick" href="javascript:void(0);" title="修改我的群名片" aria-label="修改我的群名片">修改我的群名片</a></div>\r\n    </div>\r\n</li>    \r\n<li class="right-bar">    \r\n    ';
...
},{"../../lib/config":3,"../../lib/util":7,"./card":8,"./cgi":9,"./client":10,"./list-html":14,"./tmpl/office-toolbar":25,"./tmpl/toolbar":26}],28:[function(require,module,exports){
'use strict';
...
module.exports = tdw;
},{"../../lib/callClient":2,"../../lib/report":4,"../../lib/util":7}]},{},[1]);
