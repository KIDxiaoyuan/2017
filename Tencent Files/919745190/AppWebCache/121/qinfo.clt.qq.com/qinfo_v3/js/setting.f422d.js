...
var isTim = appName && appName === 'tim';
//新ui宽度
var newui = 5509;
var newui2= 5521;
if(G.version >= newui && G.version < newui2){
    $('.main').addClass('old-version');
}else if(G.version >= newui2 ){
    //474 522
    void 0;
    $('.main').addClass('new-version');
}

if(isTim) {
    $(document.body).addClass('tim');
}
...
function setResultHander(data) {
    ...
    if (data.ec === 0) {
        ...
        //判断是否业务定制群，如果是，则隐藏编辑头衔按钮
        if(G.setting && G.setting.app_privilege_flag && (G.setting.app_privilege_flag & 0x4000)){
            try{
                document.getElementById('customLevelButton').style.display = 'none';
            }catch (e){
                void 0;
            }
...
init();
},{"../lib/config":3,"../lib/report":5,"../lib/request":6,"../lib/util":10,"../module/setting/cgi":13,"../module/setting/client":14,"../module/setting/shut":23,"../module/setting/verify":30,"../module/setting/view":31}],2:[function(require,module,exports){
'use strict';
/*
...
},{"./callClient":2,"./util":10}],6:[function(require,module,exports){
'use strict';
//ajax请求
var util = require('./util');
...
db.post = post;
db.bkn = bkn;
},{"./report":5,"./requestrep":7,"./util":10}],7:[function(require,module,exports){
'use strict';
/**
...
},{"./report":5}],8:[function(require,module,exports){
'use strict;'
var _ = {};
var escapeMap = {
    '&': '&amp;',
...
__p += '\r\n';
if(!opt.refresh){;
__p += '\r\n<span class="select-span disabled">\r\n    ';
if(opt.input){;
...
__p += '\r\n        <button class="select-span-text" role="combobox" aria-owns="' +
__e(opt.id) +
'" tabindex="1">\r\n            ';
if(!opt.empty){;
__p +=
...
},{"./config":3,"./tmpl/vcode":9}],12:[function(require,module,exports){
'use strict';
//一些UI方法
//下拉框转换
...
},{"./tmpl/select":8,"./util":10}],13:[function(require,module,exports){
'use strict';
/**
 * 增加了直出部分的兼容逻辑.by horde 2016-4-11;
 */
...
module.exports = (function() {
    ...
    function getGroupInfo(param, cb, error) {
        ...
        if(typeof nodeData !== 'undefined' && nodeData['setting']){
            cb(nodeData['setting']);
        }else{
    ...
    function getQunInfo(cb,error) {

        request.post('/cgi-bin/qun_info/get_group_info_all', {
            t : 'basic',
            gc : gc
        }, cb, error);
    ...
    //成员邀请好友加群
    function setFriendInvite(param,cb,error) {
        if (!param.gc) {
            param.gc = gc;
        }        
        request.post('/cgi-bin/qun_info/set_invite_friend', param, cb, error);        
    }
    ...
    return {
        ...
        getQunInfo : getQunInfo,
        setFriendInvite : setFriendInvite,
        init: init
    }

})();
},{"../../lib/request":6}],14:[function(require,module,exports){
'use strict';
//一些工具方法
//设置页用到的客户端方法
...
},{"../../lib/callClient":2,"../../lib/util":10}],15:[function(require,module,exports){
//aio面板 应用相关设置
var cgi = require('./cgi');
...
var qlVersion = 5445; //轻聊版本
var customLevelVersion = 5389; //什么版本可以显示自定义aio设置项目
var canShow = true; //是否可以显示aio设置
var appName = util.getParameter('appName');
var isTim = appName && appName === 'tim';
...
function init(){
    ...
    if((G.version %6 === 3 && G.version >= qlVersion)){
        ...
        canShow = false;
    }else if (G.version >= customLevelVersion && G.role === "1") {
...
},{"../../lib/util":10,"./cgi":13,"./tmpl/label":27}],16:[function(require,module,exports){
/**
 * 付费群相关设置
 * @type {[type]}
 */
...
module.exports = (function() {
	...
	function setShow(fee,flag,flag1){
		...
		if(fee){
			...
			$("#inviteMemberSetting input").prop('disabled',true).eq(0).prop('checked',true);		
		}	
		bindAction();
        //群主把可以填的恢复        
        if(G.role === "1"){
...
},{"../../lib/config":3,"../../lib/md5":4,"../../lib/report":5,"../../lib/util":10,"./cgi":13,"./client":14}],17:[function(require,module,exports){
'use strict'
//加群方式相关
//
var config = require('../../lib/config');
var client = require('./client');
var cgi = require('./cgi');
var groupEnterModule = require('./enter');    //付费入群
...
var joinOption;
var canInvite;
var friendInviteVersion = 5509; //8.8 5509 版本开始显示
var friendInvite = 0;
...
function setInvert(e) {
    ...
    G.tdwList.push(tdw);
    var tdw1 = {}
    tdw.module = 'invite_mana';
    tdw.action = flag ? 'mana_open_invite' : 'mana_close_invite';
    G.tdwList.push(tdw1);

    var ret = client.setGroupMemberInviteOption(flag);
    changeFriendInvite(flag);
}

//变更允许好友加群状态
function changeFriendInvite(flag){
    //canInvite = flag;
    $('#allowInvite').prop('checked', flag);
    if($('#needEnter').prop('checked')){
        $("#inviteMemberSetting input").prop('disabled',true);
    }else{
        $("#inviteMemberSetting input").prop('disabled',!flag);    
    }
}

//初始化邀请好友加群
function initInviteJoin(flag){
    friendInvite = flag;
    if(G.version >= friendInviteVersion){
        if(G.role === '1' || G.role === '2'){
            $("#inviteMemberSetting").find('input[value="'+flag+'"]').prop('checked',true);
        }else{
            $("#inviteSetting").remove();
        }
    }else{
        $("#inviteSetting").remove();
    }
...
var inviteMap = {
    0 : 'mana_need_auditing',
    2 : 'mana_autojoin_50',
    3 : 'mana_autojoin_100',
    1 : 'mana_noneed_auditing'
}

//设置成员邀请好友加群
function setInviteJoin(e){
    var target = $(e.target);
    ...
    tdw.module = 'invite_mana';
    tdw.action = inviteMap[value];
    G.tdwList.push(tdw);

    cgi.setFriendInvite(param,function(res){
        if(res.ec === 0){
            friendInvite = value;  
            
        }else{
            $("#inviteMemberSetting").find('input[value="'+friendInvite+'"]').prop('checked',true);
        }
    },function(res){
...
function changeQunSearchStatus(val){
    joinOption = val;
    void 0;
    $("body").trigger('qunsearchstatus',val===3?false:true);
    if(val === 3 || val === 1){
        ...
        $("#inviteMemberSetting input").prop('disabled',true);
        /*
        1、允许任何人，邀请方式变成无需审核直接进群
        3、不允许任何人，邀请开关关闭
         */
        if(val === 1){
            if(!$('#allowInvite').prop('checked')){
                $('#allowInvite').click();
            }
            $("#allowInvite").prop('disabled',false).prop('checked',true);
            $("#inviteMemberSetting input").eq(0).prop('checked',true); 
        }else if(val === 3){
            if($('#allowInvite').prop('checked')){
                $('#allowInvite').click();
            }            
            $("#allowInvite").prop('disabled',true)
            $("#inviteMemberSetting input").prop('checked',false);  
        }else{
            $("#inviteMemberSetting").find('input[value="'+friendInvite+'"]').prop('checked',true);
        }
    }else if(val === 6){
        ...
        $("#allowInvite").prop('disabled',false);
        if($('#allowInvite').prop('checked')){
            $("#inviteMemberSetting input").prop('disabled',false);    
        }else{
            $("#inviteMemberSetting input").prop('disabled',true);    
        }
...
function joinGroup() {
    ...
    changeFriendInvite(flag);
    changeQunSearchStatus(joinOpt.option);
    switch (joinOpt.option) {
        case 1: //允许任何人加入
            $('#allowAll').prop('checked', true).attr('data-def', 1).parents('.can-hide').removeClass('can-hide');
            break;
        case 2: //需要验证消息
            $('#needIdentity').prop('checked', true).attr('data-def', 1).parents('.can-hide').removeClass('can-hide');
            break;
        case 3: // 不允许任何人加入
            $('#notAllowJoinGroup').prop('checked', true).attr('data-def', 1).parents('.can-hide').removeClass('can-hide');
            break;
        case 4: //需要正确回答问题
...
function setJoin(e) {
    ...
    tdw.ver2 = val;
    //付费入群修改
    groupEnterModule.changeStatus(val);
    //把允许好友加群的按钮灰掉

    changeQunSearchStatus(val);
...
//变更提示窗的状态
function changeAlert(flag){
    return alertTips = flag;
}

//变更加群设置的状态
function changeOpt(param){
    return setJoinOpt = param;
}

function getGroupInfo(res){
    if(res.ec === 0){
        if(res.gtype === 2){
            //同城群状态下不能勾选此项
            $("#allowQunSearch").prop('checked',false).prop('disabled',true);
            $("#allowQunSearchLabel").text('只能通过群号找到这个群 （同城群状态下不能勾选此项）');

            //console.log($("#notAllowJoinGroup").prop('checked'),$(".identity-verify-input:checked").length);

            if($("#notAllowJoinGroup").prop('checked') || $(".identity-verify-input:checked").length === 0){
                $("#needIdentity").prop('checked',true);
            }            
            $("#notAllowJoinGroup").prop('checked',false).prop('disabled',true);
        }
    }
}
...
//设置能否通过群号查找
function init(obj){
    cgi.getQunInfo(getGroupInfo,getGroupInfo);

    void 0;
    initInviteJoin(obj.invite_friend);

    //允许查找到群
    if(G.role === '3' && obj.search !== 1){
    ...
    if(G.role === "2"){
        ...
        $("#groupVerify").addClass('disabled-gray');
    }
    ...
    // if(obj.inner){
    //     //identityAll
    //     $("#identityInner").removeClass('hide');
    // }else{
        //$("#identityInner").removeClass('hide');
        if($("#identityInner input").prop('checked')){
            //$("#needIdentity").click();
        }
        //$("#identityInner").remove();
    // /}

    //只能通过群号查找
    if (obj.search === 1) {
        $("#allowQunSearch").prop('checked', true);
    }
}

//更改群号查找按钮点击状态
function checked(flag){
    $("#allowQunSearch").prop('checked', flag);
}
...
module.exports = {
    init : init,
    setQunSearch : setQunSearch,
    checked : checked,
    getStatus : getStatus,
    checkSelectInput2 : checkSelectInput2,
    checkSelectInput : checkSelectInput,
    checkAndSaveOption : checkAndSaveOption,
    setAnswer : setAnswer,
    setInvert : setInvert,
    joinGroup : joinGroup,
    setJoin : setJoin,
    changeAlert : changeAlert,
    setInviteJoin : setInviteJoin,
    changeOpt : changeOpt
}
},{"../../lib/config":3,"./cgi":13,"./client":14,"./enter":16}],18:[function(require,module,exports){
'use strict'
/**
 * 成员等级设置和标签相关 成员信息
 * @type {[type]}
...
function init(obj){
    ...
    //显示成员等级
    if (obj.levelflag === 1) {
        $('#showMemberLevel').attr('checked', true);
    }
...
},{"../../lib/config":3,"../../lib/util":10,"./cgi":13,"./client":14}],19:[function(require,module,exports){
'use strict'
/*群消息提示相关设置*/
var client = require('./client');
var util = require('../../lib/util');
var msgVersion = 5461; //5461 生效的版本 . 消息提示修改 改ui的时候临时改成个很大的值.
...
//tim消息设置显示
function timMsgSetting(opt) {
    $('#timMsgSettingDl').removeClass('hide');
    $("#msgSettingDl").addClass('hide');
    
    switch (opt) {
        case 1:
            $('#timMobileAuto').prop('checked', true).attr('data-def', 1);
            break;
        case 2:
            $('#timGroupNotips').prop('checked', true).attr('data-def', 1);
            break;
        /*case 3:
            $('#mobileShield').prop('checked', true).attr('data-def', 1);
            break;*/
        case 4:
            $('#timReceiveNotips').prop('checked', true).attr('data-def', 1);
            break;
    }
}
...
},{"../../lib/util":10,"./client":14}],20:[function(require,module,exports){
'use strict'
//游客权限
var cgi = require('./cgi');
var config = require('../../lib/config');
 ...
 */
function init(obj){
    if (parseInt(G.role) > 2 && !obj.share) {
    ...
    var canShare = $('#visitorShareAccess');
...
},{"../../lib/config":3,"../../lib/vcode":11,"./cgi":13}],21:[function(require,module,exports){
/**
 * 图片屏蔽相关设置
 * @type {[type]}
 */
...
},{"../../lib/config":3,"../../lib/util":10,"./client":14}],22:[function(require,module,exports){
'use strict'
//退群
...
function init(){
    ...
    if(G.version >= exitGroupVersion && G.role === '1'){
...
module.exports = (function() {
    ...
    var shutDom = $('#speakShowBlock');
    var nowShutTime = -1; //禁言时间
    var allShutDom = $("#allShutBlock");
    //var shutListDom = $("#speakShowBlock");
    var isAlert = false;
...
},{"../../lib/config":3,"../../lib/widget":12,"./cgi":13,"./client":14,"./tmpl/dialog":26,"./tmpl/shut":28,"./view":31}],24:[function(require,module,exports){
/*会话权限相关设置*/
/**
...
var phoneVersion = 5973; //qq电话设置入口.

var dom = $('#phoneShow');

/*
todo  qq电话这儿设置好像有问题.明天确认下
 */
...
function getDiscuss(){
    var handler = function(res){
        var disTxt = '群内禁止普通群成员（不含群主和管理员）发起临时会话', //'群内禁止普通群成员（不含群主和管理员）发起临时会话',//'禁止群内发起临时会话',
            groupTxt = '群内禁止普通群成员（不含群主和管理员）发起多人聊天'; //'群内禁止普通群成员（不含群主和管理员）发起多人聊天';//'禁止群内发起讨论组';
        if(!parseInt(res.forbid_discuss)){
            groupTxt = '允许群成员发起多人聊天';
        }
        ...
        if(res.ec === 0){
            if(G.role === '1'){
                $("#allowtempDiscuss").prop('checked',res.forbid_chat == 0?true:false);//.attr('aria-label',disTxt);
                $("#allowtempGroup").prop('checked',res.forbid_discuss == 0?true:false);//attr('aria-label',groupTxt);
            }else if(G.role === '2'){                
                $("#allowtempDiscuss").prop('disabled',true).prop('checked',res.forbid_chat == 0?true:false);
                $("#allowtempGroup").prop('disabled',true).prop('checked',res.forbid_discuss == 0?true:false);
            }else{
    ...
    */
    ...
    var handler = function(data) {
        if (data.retcode === 0) {
            ...
            if (!(G.role === '1' || G.role === '2')) {
                $('#anonymousTalkDl input').remove();
                var lable = $('#anonymousTalkDl label').eq(0);
                //不允许
...
module.exports = {
    ...
    getDiscuss : getDiscuss,
    setGroup : setGroup ,
    setQunVideo : setQunVideo   
}
},{"../../lib/config":3,"../../lib/report":5,"../../lib/util":10,"./cgi":13,"./client":14}],25:[function(require,module,exports){
'use strict;'
var _ = {};
var escapeMap = {
    '&': '&amp;',
...
function print() { __p += __j.call(arguments, '') }
    ...
    var ico = locals.ico;
    var msg = locals.msg;
    var select = locals.select;
;
__p += '\r\n        <div class="hd" id="shutDialogTit">' +
__e(tit) +
...
var tabs = locals.tabs;
var isTim = locals.isTim;
var sel_id = locals.sel_id;
for(var i = 0,l=tabs.length;i<l;i++){
    var item = tabs[i];
	var id = item && item.id && parseInt(item.id, 10);
	if(isTim && (id === 1 || id === 3)) {
		continue;
	}
;
...
+item.id;
__p += '" aria-label="在聊天面板显示' +
__e(item.wording) +
'" ';
if(item.id===sel_id){;
__p += 'checked';
};
__p += ' tabindex="1" /><label for="cLabel' +
((__t = (item.id)) == null ? '' : __t) +
'">' +
__e(item.wording) +
'</label>\r\n\t\t';
...
},{"../../lib/config":3,"./cgi":13}],30:[function(require,module,exports){
'use strict';
/*
加群验证相关设置
 */
...
},{"./cgi":13,"./tmpl/adminlist":25,"./view":31}],31:[function(require,module,exports){
'use strict';
/**
 * change by horde 2016.3.24   http://tapd.oa.com/QQGroup/prong/stories/view/1010076071057495843
 * 调整ui布局.顺便把各个模版的功能单独拆分出去.这里保留ui设置上的部分逻辑.
 */
...
/*各个功能模块*/
var pic = require('./pic');             //屏蔽图片
var talkModule = require('./talk'); //匿名聊
var groupEnterModule = require('./enter');    //付费入群
//var groupSearchModule = require('./groupsearch'); //是否只能通过群号查找
var uploadModule = require('./upload'); //上传权限设置
var customLabelModule = require('./customlabel'); //aio面板应用设置
var quitModule = require('./quitgroup'); //退群和解散群
var openModule = require('./open'); //游客相关权限
var memberModule = require('./member'); //成员模块
var messageModule = require('./message'); //消息提醒相关设置
var joinModule = require('./join'); //加群相关设置
/* end */
...
module.exports = (function() {
    ...
    function bindEvent() {
        var task = {
            ...
            'view.uploadFile': uploadModule.upload,
            'view.setJoin': joinModule.setJoin,
            'view.setMsgStatus': messageModule.setMsgStatus,
            'view.setInvert': joinModule.setInvert,
            'view.setQunVideo': talkModule.setQunVideo,
            'view.setQunSearch': joinModule.setQunSearch,
            'view.setAnswer': joinModule.setAnswer,
            'view.setOpen': openModule.setOpen,
            'view.setGroup': openModule.setGroup,
            'view.setLabel': customLabelModule.set,
            'view.setAnonymous': talkModule.set, //匿名聊天
            'view.setFollow' : setFollow,
            'view.setPic' : pic.set,
            'view.exitGroup' : exitGroup,
            'view.enterMore' : groupEnterModule.more,
            'view.enterPay' : groupEnterModule.pay,
            'view.setQunPhone' : talkModule.setPhone,
            'view.setDiscuss' : talkModule.setDiscuss,
            'view.setdisGroup' : talkModule.setGroup,
            'view.friendinvite' : joinModule.setInviteJoin         
        }
        $.extend(G.task, task);
    }
    ...
    function resetUi(role) {
        //把下拉框替换成自定义的样式
        widget.transSelect($('#interval'), messageModule.setMsgStatus);
        widget.transSelect($('#setQuestion'), function(value, name) {
            ...
            joinModule.checkAndSaveOption(param);
        }, {
            input: true,
            empty: true,
            checkfn: joinModule.checkSelectInput,
            inputmax: 30
        });
        widget.transSelect($('#setQuestion2'), function(value, name) {
            ...
            joinModule.checkAndSaveOption(setJoinOpt);
            $('#setAnswerInput').attr('data-value', value);
        }, {
            input: true,
            checkfn: joinModule.checkSelectInput2,
            empty: true,
            inputmax: 30
        });
        ...
        //因为初始化需要时间.所以调整一下时序
        //群消息设置
        messageModule.msgSetting();
        //群视频设置
        talkModule.setVideo();
        //加群设置
        joinModule.joinGroup();
    }
    ...
    function changeAccess(obj) {
        ...
        /**
         * change by horde 2016.3.24 
         * 把一堆原来写在这儿的逻辑拆分到各个具体的模块里面.
         */
        ...
        if (G.role === "1") {
            ...
            // tim
            $('#timMsgSettingDl .disabled').removeClass('.disabled');
            $('#timMsgSettingDl input').prop('disabled', false);
        ...
        //成员设置
        memberModule.init(obj);
        //显示应用权限设置
        uploadModule.init(obj);
        //aio应用lab设置
        customLabelModule.initLabel(obj.aio_tabs);
        //只能通过群号查找相关设置
        joinModule.init(obj);
        //游客权限设置
        openModule.init(obj);       
    ...
    function init() {
        ...
        talkModule.init();    //匿名聊天初始化
        customLabelModule.init();  //aio标签初始化
        quitModule.init();         //退群初始化
    ...
    function closeFn() {
        var opt = joinModule.getStatus();

        if (opt.alert) {
            client.common.alert(2, config.msg.tit.alert, config.msg.sensitive.setting);
            joinModule.changeAlert(false);
            return 0;
        } else {
            var setJoinOpt = opt.joinOpt;
            if (setJoinOpt) {
    ...
    $('body').bind('qunsearchstatus',function(e,d){
        qunSearchStatus = d;
    });
...
},{"../../lib/config":3,"../../lib/report":5,"../../lib/util":10,"../../lib/vcode":11,"../../lib/widget":12,"./cgi":13,"./client":14,"./customlabel":15,"./enter":16,"./join":17,"./member":18,"./message":19,"./open":20,"./pic":21,"./quitgroup":22,"./talk":24,"./upload":29}]},{},[1]);
